    ...
    <Grid
      ...
      <Grid
        ...
        {isLoading ? (
          ...
          (() => {
            const filteredTickets = ticketsForSale.filter(
              (ticket: ITicketFromContract) =>
                ticket.ticketOwner.toLowerCase() !== account.toLowerCase()
            );
            if (filteredTickets.length === 0) {
              return <div>Žádné vstupenky k prodeji.</div>;
            } else {
              return filteredTickets.map((ticket: ITicketFromContract, index: number) => {
